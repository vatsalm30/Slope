...
# camera positions: apex = vertex[1] of each frustum (geometry_1 … geometry_11)
cameras = np.array([scene.geometry[f"geometry_{i}"].vertices[1]
                    for i in range(1, 12)], dtype=np.float64)

# for each camera, find the terrain point directly below it (nearest in XZ plane)
...
# ── pairwise slopes between consecutive blue points ───────────────────────────
# Order cameras by along-flight position so "consecutive" matches flight order.
s_all = (blue_pts[:, [0, 2]] - blue_pts[:, [0, 2]].mean(0)) @ flight_dir
order = np.argsort(s_all)          # ascending along-path index

blue_ordered = blue_pts[order]     # all 11 (including any outliers)
ok_ordered   = ok[order]
cam_ordered  = cameras[order]
...
for i in range(len(blue_ordered) - 1):
    ...
    horiz  = np.linalg.norm(b[[0, 2]] - a[[0, 2]])   # XZ distance
    dY     = abs(b[1] - a[1])                          # terrain height change (+Y=down)
    slope  = np.degrees(np.arctan(dY / horiz)) if horiz > 1e-6 else float("nan")
    ia, ib = order[i] + 1, order[i + 1] + 1           # 1-based camera numbers
...
plane_verts = [[c[0], c[1], c[2]] for c in corners]   # (X, Z, Y)
...
# --- right: pairwise slope bar chart ---
...
plt.suptitle(f"VGGT 11-view — Happy Hollow terrain slope", fontsize=12, y=1.01)
plt.tight_layout()
plt.savefig("slope_result.png", dpi=150, bbox_inches="tight")
print(f"\nPlot saved → slope_result.png")
